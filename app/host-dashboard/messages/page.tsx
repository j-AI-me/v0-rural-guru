...
export default function MessagesPage() {
  ...
  const getReceiverId = () => {
    ...
    // Si el usuario es anfitrión, el receptor es el huésped
    if (user?.role === "host") {
      return bookingDetails.guest_id
    }
    // Si el usuario es huésped, el receptor es el anfitrión
        ...
        <div className="grid md:grid-cols-[350px_1fr] gap-8 min-h-[600px]">
          <Card className="md:max-h-[700px] overflow-hidden">
            <CardHeader>
              ...
              <CardDescription>Tus conversaciones con huéspedes</CardDescription>
            </CardHeader>
            ...
            {selectedBooking && bookingDetails ? (
              <>
                <CardHeader className="pb-2">
                  <CardTitle>
                    {user.role === "host"
                      ? bookingDetails.guest?.full_name || "Huésped"
                      : bookingDetails.properties?.name || "Propiedad"}
                  </CardTitle>
                  <CardDescription>
